...
function cancelSubscription(platform, payment, cb) {
  ...
  if (!engine.cancelSubscription) {
    return syncError(
      new Error(`Platform ${platform} does not have cancelSubscription method`)
    );
  }
...
exports.verifyPayment = (platform, payment, cb) => {
  return cb
    ? verifyPayment(platform, payment, cb)
    : promisify(verifyPayment)(platform, payment);
};

exports.cancelSubscription = (platform, payment, cb) => {
  return cb
    ? cancelSubscription(platform, payment, cb)
    : promisify(cancelSubscription)(platform, payment);
};
